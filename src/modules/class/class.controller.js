...
export class ClassController {
    ...
    static findById = tryCatchWrapper(async function(req, res) {
        let result = await ClassService.findById(req.params.id_class)
        return res.status(200).json(result)
    })

    static findAllStats = tryCatchWrapper(async function(req, res) {
        let result = await ClassService.findStats({...req.query})
        return res.status(200).json(result)
    })

    static findStatsById = tryCatchWrapper(async function(req, res) {
        let activeSemester = await new Semester
        let result = await ClassService.findStatsById({id:req.params.id_class})
        return res.status(200).json(result)
    })
}